...
impl ProviderCapabilities {
    pub fn text_only() -> Self {
        Self { text_chat: true, native_tools: false, streaming: false, reasoning: false }
    }
}
...
fn anthropic(
    ...
    for block in body.content {
        match block.kind.as_str() {
            ...
            "tool_use" => {
                let id = block.id.unwrap_or_else(|| format!("anthropic_{tool_index}"));
                let name = block.name.unwrap_or_default();
                let input = block.input.unwrap_or(serde_json::Value::Object(Default::default()));
                native_tool_calls.push(NativeToolCall { id, name, arguments: input });
                tool_index += 1;
            }
            _ => {}
        }
