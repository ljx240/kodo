...
impl TaskPlan {
    ...
    pub fn from_task(message: &str) -> Self {
        ...
        let mut subtasks = vec![Subtask::new(
            "s1",
        ...
        if mutating {
            ...
            subtasks.push(Subtask::new(
                "s3",
                "Verify the project still holds",
                SubtaskKind::Verify,
            ));
        }
    ...
    pub fn evaluate_acceptance(&self, evidence: &AcceptanceEvidence) -> AcceptanceReport {
        ...
        if self
            .subtasks
            .iter()
            .any(|s| s.kind == SubtaskKind::Edit && !s.done)
        {
        ...
        if evidence.model_claimed_done
            && failures.is_empty()
            && passed.len() == self.acceptance_criteria.len()
        {
